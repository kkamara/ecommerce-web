...
class CommonHelper
{
    ...
    /**
     * Sanitise given input of any type.
     * 
     * @param  any $subject
     * @return any
     */
    public static function sanitise($subject) {
        $result = null;

        switch(gettype($subject)) {
            case "integer":
                $result = filter_var($subject, FILTER_SANITIZE_NUMBER_INT);
                break;
            case "double":
                $result = filter_var($subject, FILTER_SANITIZE_NUMBER_FLOAT);
                break;
            case "string":
                $result = filter_var($subject, FILTER_SANITIZE_STRING);
                break;
            case "array":
                $result = array();
                foreach($subject as $k => $val) {
                    $result[$k] = self::sanitise($val);
                }
                break;
            case "object":
                foreach ($subject as $value) {
                    if (false === is_scalar($value)) continue;
